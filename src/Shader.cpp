...
namespace maple
{
    ...
    auto Shader::spirvTypeToDataType(const spirv_cross::SPIRType& type,
                                     uint32_t size) -> ShaderDataType
    {
    ...
    auto Shader::spirvTypeToTextureType(const spv::ImageFormat& format)
      -> TextureFormat
    {
        switch (format)
        {
		...
		throw std::runtime_error("unsupported spv::ImageFormat");
    }
      ...
      std::unordered_multimap<ShaderType, std::string>& shaders) -> void
    {
        for (uint32_t i = 0; i < lines.size(); i++)
        {
            ...
            if (StringUtils::startWith(str, "#"))
            {
                ...
                auto type =
                  getShaderTypeByName(path[0].substr(1, path[0].length()));
