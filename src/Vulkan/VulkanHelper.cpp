...
namespace maple
{
    namespace
    {
        ...
        inline auto layoutToAccessMask(const VkImageLayout layout,
                                       const bool isDestination)
          -> VkPipelineStageFlags
        {
            ...
            switch (layout)
            {
                case VK_IMAGE_LAYOUT_UNDEFINED:
                    if (isDestination)
                    {
                        LOGE("The new layout used in a transition must not be "
        ...
        inline auto accessFlagsToPipelineStage(
          ...
          const VkPipelineStageFlags stageFlags)
        {
            ...
            while (accessFlags != 0)
            {
                VkAccessFlagBits newAccessFlag = static_cast<VkAccessFlagBits>(
                  accessFlags & (~(accessFlags - 1)));
                MAPLE_ASSERT(newAccessFlag != 0 &&
                               (newAccessFlag & (newAccessFlag - 1)) == 0,
                             "Error in access flags");
    ...
    auto VulkanHelper::findMemoryType(uint32_t typeFilter,
                                      VkMemoryPropertyFlags properties)
      -> uint32_t
    {
        VkPhysicalDeviceMemoryProperties memProperties;
        vkGetPhysicalDeviceMemoryProperties(
          *VulkanDevice::get()->getPhysicalDevice(), &memProperties);
    ...
    auto VulkanHelper::querySwapChainSupport(VkPhysicalDevice device,
    ...
    auto VulkanHelper::checkDeviceExtensionSupport(
    ...
    auto VulkanHelper::checkValidationLayerSupport(
      const std::vector<const char*>& layerNames) -> bool
    {
        uint32_t layerCount;
    ...
    auto VulkanHelper::copyTo(const Texture2D::Ptr& from,
                              Texture2D::Ptr& to,
                              const CommandBuffer* cmdBuffer) -> void
    {
        ...
        vkFrom->transitionImage(
          VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
          static_cast<const VulkanCommandBuffer*>(cmdBuffer));
        vkTo->transitionImage(
        ...
        vkCmdCopyImage(static_cast<const VulkanCommandBuffer*>(cmdBuffer)
                         ->getCommandBuffer(),
                       vkFrom->getImage(),
                       VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                       vkTo->getImage(),
        ...
        vkFrom->transitionImage(
          vkFromLayout, static_cast<const VulkanCommandBuffer*>(cmdBuffer));
        vkTo->transitionImage(
          vkToLayout, static_cast<const VulkanCommandBuffer*>(cmdBuffer));
    }
    ...
    auto VulkanHelper::validateResolution(uint32_t& width, uint32_t& height)
      -> void
    {
        VkSurfaceCapabilitiesKHR capabilities;
        auto surface = std::static_pointer_cast<VulkanSwapChain>(
                         GraphicsContext::get()->getSwapChain())
                         ->getSurface();
        vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
          *VulkanDevice::get()->getPhysicalDevice(), surface, &capabilities);
    ...
    auto VulkanHelper::instanceCreateInfo(
      const VkApplicationInfo& appInfo,
      const std::vector<const char*>& extensions,
      const std::vector<const char*>& validationLayers,
      bool validationLayer) -> VkInstanceCreateInfo
    {
        // Create Info
    ...
    auto VulkanHelper::getApplicationInfo(const std::string& name)
      -> VkApplicationInfo
    {
    ...
    auto VulkanHelper::findQueueFamilies(VkPhysicalDevice physicalDevice,
                                         VkSurfaceKHR surface)
      -> QueueFamilyIndices
    {
    ...
    auto VulkanHelper::createBuffer(
      VkBuffer& buffer,
      ...
      const std::vector<uint32_t>& queueFamilyIndices /*= {}*/) -> void
    {
    ...
    auto VulkanHelper::physicalDeviceTypeString(VkPhysicalDeviceType type)
      -> std::string
    {
        switch (type)
        {
...
#undef STR
    ...
    auto VulkanHelper::setupDebugging(VkInstance instance,
                                      VkDebugReportFlagsEXT flags,
                                      VkDebugReportCallbackEXT callBack) -> void
    {
    ...
    auto VulkanHelper::populateDebugMessengerCreateInfo(
    ...
    auto VulkanHelper::getSupportedFormat(
      ...
      VkFormatFeatureFlags features) -> VkFormat
    {
        for (VkFormat format : candidates)
        {
            ...
            vkGetPhysicalDeviceFormatProperties(
              *VulkanDevice::get()->getPhysicalDevice(), format, &props);
    ...
    auto VulkanHelper::setObjectName(const std::string& name,
                                     uint64_t handle,
                                     VkObjectType objType) -> void
    {
        if (name != "" && handle != 0)
        {
    ...
    auto VulkanHelper::getSupportedDepthFormat(VkPhysicalDevice physicalDevice,
    ...
    auto VulkanHelper::chooseSwapSurfaceFormat(
    ...
    auto VulkanHelper::chooseSwapPresentMode(
      ...
      bool vsync) -> VkPresentModeKHR
    {
        ...
        if (!vsync)
        {
            for (size_t i = 0; i < availablePresentModes.size(); i++)
            {
                ...
                /*	if(availablePresentModes[i] ==
                 * VK_PRESENT_MODE_IMMEDIATE_KHR) { swapchainPresentMode =
    ...
    auto VulkanHelper::chooseSwapExtent(
    ...
    auto VulkanHelper::createImageView(VkImage image,
                                       VkFormat format,
                                       uint32_t mipLevels,
                                       ...
                                       uint32_t baseMipLevel) -> VkImageView
    {
        PROFILE_FUNCTION();
        ...
        if (vkCreateImageView(
              *VulkanDevice::get(), &viewInfo, nullptr, &imageView) !=
            VK_SUCCESS)
        {
    ...
    auto VulkanHelper::createImageVma(const VkImageCreateInfo& imageInfo,
                                      VkImage& image,
                                      VmaAllocation& allocation) -> void
    {
        PROFILE_FUNCTION();
        ...
        VK_CHECK_RESULT(vmaCreateImage(VulkanDevice::get()->getAllocator(),
                                       &imageInfo,
                                       &allocInfovma,
                                       &image,
                                       &allocation,
...
#else
    auto VulkanHelper::createImageDefault(const VkImageCreateInfo& imageInfo,
                                          VkImage& image,
      ...
      -> void
    {
        ...
        vkGetImageMemoryRequirements(
          *VulkanDevice::get(), image, &memRequirements);
        ...
        vkAllocateMemory(
          *VulkanDevice::get(), &allocInfo, nullptr, &imageMemory);
        vkBindImageMemory(*VulkanDevice::get(), image, imageMemory, 0);
    }
#endif

#ifdef USE_VMA_ALLOCATOR
    auto VulkanHelper::createImage(uint32_t width,
                                   uint32_t height,
                                   uint32_t mipLevels,
                                   VkFormat format,
...
#else
        VulkanHelper::createImageDefault(
          imageInfo, image, imageMemory, properties);
#endif
    }

    auto VulkanHelper::transitionImageLayout(VkImage image,
                                             VkFormat format,
                                             ...
                                             uint32_t baseMipLevel) -> void
    {
        ...
        VkImageMemoryBarrier imageMemoryBarrier =
          VulkanHelper::imageMemoryBarrier();
        ...
        if (cmd != nullptr &&
            cmd->getCommandBuffeType() == CommandBufferType::Compute)
        {
            imageMemoryBarrier.srcQueueFamilyIndex = VulkanDevice::get()
                                                       ->getPhysicalDevice()
                                                       ->getQueueFamilyIndices()
                                                       .computeFamily.value();
            imageMemoryBarrier.dstQueueFamilyIndex = VulkanDevice::get()
                                                       ->getPhysicalDevice()
                                                       ->getQueueFamilyIndices()
                                                       .computeFamily.value();
        }
    ...
    auto VulkanHelper::beginSingleTimeCommands() -> VkCommandBuffer
    {
        ...
        vkAllocateCommandBuffers(
          *VulkanDevice::get(), &allocInfo, &commandBuffer);
    ...
    auto VulkanHelper::endSingleTimeCommands(VkCommandBuffer commandBuffer)
      -> void
    {
        ...
        VK_CHECK_RESULT(vkQueueSubmit(VulkanDevice::get()->getGraphicsQueue(),
                                      1,
                                      &submitInfo,
                                      VK_NULL_HANDLE));
        VK_CHECK_RESULT(
        ...
        vkFreeCommandBuffers(*VulkanDevice::get(),
                             *VulkanDevice::get()->getCommandPool(),
                             1,
                             &commandBuffer);
    }

    auto VulkanHelper::copyBufferToImage(VkBuffer buffer,
                                         VkImage image,
                                         uint32_t width,
                                         uint32_t height,
                                         uint32_t depth,
                                         ...
                                         int32_t offsetY,
                                         int32_t offsetZ,
                                         const VulkanCommandBuffer* cmd) -> void
    {
        VkCommandBuffer commandBuffer =
          cmd == nullptr ? beginSingleTimeCommands() : cmd->getCommandBuffer();
    ...
    auto VulkanHelper::setImageLayout(VkCommandBuffer cmdbuffer,
                                      VkImage image,
    ...
    auto VulkanHelper::createTextureSampler(
      ...
      -> VkSampler
    {
        ...
        VK_CHECK_RESULT(vkCreateSampler(
          *VulkanDevice::get(), &samplerInfo, nullptr, &sampler));
        return sampler;
    }

    namespace VkConverter
    {
        auto textureFormatToVK(const TextureFormat& format,
                               bool srgb,
                               bool ignoreAssert) -> VkFormat
        {
            if (srgb)
            {
                switch (format)
                {
                    ...
                    default:
                        MAPLE_ASSERT(ignoreAssert,
                                     "[Texture] Unsupported image bit-depth!");
        ...
        auto textureMipmapFilterToVK(TextureFilter filter)
          -> VkSamplerMipmapMode
        {
            switch (filter)
            {
        ...
        auto shaderTypeToVK(const ShaderType& shaderName)
          -> VkShaderStageFlagBits
        {
            ...
            for (auto i = 0; i < 11; i++)
            {
                if (((uint32_t)shaderName & (uint32_t)flags[i]) ==
                    (uint32_t)flags[i])
                {
                    retBits |= types[i];
                }
