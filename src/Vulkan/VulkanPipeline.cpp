...
namespace maple
{
    namespace
    {
        ...
        inline auto convertBlend(BlendMode mode)
        {
            switch (mode)
            {
                case maple::BlendMode::None:
                    MAPLE_ASSERT(false, "Unsupported");
                    break;
                case maple::BlendMode::Zero:
                    return VK_BLEND_FACTOR_ZERO;
                case maple::BlendMode::One:
                    return VK_BLEND_FACTOR_ONE;
                case maple::BlendMode::SrcColor:
                    return VK_BLEND_FACTOR_SRC_COLOR;
                case maple::BlendMode::OneMinusSrcColor:
                    return VK_BLEND_FACTOR_ONE_MINUS_SRC_COLOR;
                case maple::BlendMode::SrcAlpha:
                    return VK_BLEND_FACTOR_SRC_ALPHA;
                case maple::BlendMode::OneMinusSrcAlpha:
                    return VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA;
                case maple::BlendMode::DstAlpha:
                    return VK_BLEND_FACTOR_DST_ALPHA;
                case maple::BlendMode::OneMinusDstAlpha:
                    return VK_BLEND_FACTOR_ONE_MINUS_DST_ALPHA;
                case maple::BlendMode::DstColor:
                    return VK_BLEND_FACTOR_DST_COLOR;
                case maple::BlendMode::OneMinusDstColor:
                    return VK_BLEND_FACTOR_ONE_MINUS_DST_COLOR;
                case maple::BlendMode::SrcAlphaSaturate:
            ...
            switch (type)
            {
                ...
                case maple::StencilType::Zero:
                    throw std::logic_error(
                      "Compare Op should not be Keep/Replace/Zero");
            }
        ...
        inline auto createDepthStencil(
        ...
        inline auto createMultisample(VkPipelineMultisampleStateCreateInfo& ms)
          -> void
        {
        ...
        inline auto createVertexLayout(
          ...
          std::shared_ptr<VulkanShader> vkShader) -> void
        {
            ...
            auto& vertexInputAttributeDescription =
              vkShader->getVertexInputAttributeDescription();

            vi.sType =
              VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
            vi.pNext = NULL;
            vi.vertexBindingDescriptionCount =
              vertexBindingDescription.stride != 0 ? 1 : 0;
            vi.pVertexBindingDescriptions = vertexBindingDescription.stride != 0
        ...
        inline auto createRasterization(
          ...
          const PipelineInfo& info) -> void
        {
            ...
            inputAssemblyCI.topology =
              VkConverter::drawTypeToTopology(info.drawType);
            ...
            // VK_FRONT_FACE_COUNTER_CLOCKWISE; ////info.flipY &&
            // info.swapChainTarget ? VK_FRONT_FACE_CLOCKWISE :
        ...
        inline auto createColorBlend(
          VkPipelineColorBlendStateCreateInfo& cb,
          std::vector<VkPipelineColorBlendAttachmentState>& blendAttachState,
          const PipelineInfo& info,
          std::shared_ptr<VulkanRenderPass> renderPass) -> void
        {
            ...
            for (unsigned int i = 0; i < blendAttachState.size(); i++)
            {
                ...
                {
                    blendAttachState[i].srcAlphaBlendFactor =
                      convertBlend(info.blendMode);
                    blendAttachState[i].dstAlphaBlendFactor =
                      convertBlend(info.dstBlendMode);
                    blendAttachState[i].srcColorBlendFactor =
                      convertBlend(info.blendMode);
                    blendAttachState[i].dstColorBlendFactor =
                      convertBlend(info.dstBlendMode);
                }
            }
        ...
        inline auto createViewport(VkPipelineViewportStateCreateInfo& vp,
    ...
    VulkanPipeline::~VulkanPipeline()
    {
        ...
        deletionQueue.emplace(
          [pipeline] {
              vkDestroyPipeline(*VulkanDevice::get(), pipeline, VK_NULL_HANDLE);
          });
    }
    ...
    auto VulkanPipeline::init(const PipelineInfo& info) -> bool
    {
        ...
        createColorBlend(
          ...
          blendAttachState,
          info,
          std::static_pointer_cast<VulkanRenderPass>(renderPass));
        ...
        dynamicStateCI.dynamicStateCount =
          uint32_t(dynamicStateDescriptors.size());
        dynamicStateCI.pDynamicStates = dynamicStateDescriptors.data();
        ...
        graphicsPipelineCreateInfo.stageCount =
          vkShader->getShaderStages().size();
        graphicsPipelineCreateInfo.renderPass =
          *std::static_pointer_cast<VulkanRenderPass>(renderPass);
        graphicsPipelineCreateInfo.subpass = 0;

        VK_CHECK_RESULT(
          vkCreateGraphicsPipelines(*VulkanDevice::get(),
                                    VulkanDevice::get()->getPipelineCache(),
                                    1,
                                    &graphicsPipelineCreateInfo,
                                    VK_NULL_HANDLE,
                                    &pipeline));

        VulkanHelper::setObjectName(
          info.pipelineName, (uint64_t)pipeline, VK_OBJECT_TYPE_PIPELINE);
    ...
    auto VulkanPipeline::beginSecondary(const CommandBuffer* commandBuffer,
                                        uint32_t layer /*= 0*/,
                                        int32_t cubeFace /*= -1*/,
                                        int32_t mipMapLevel /*= 0*/) -> void
    {
        PROFILE_FUNCTION();
        ...
        if (description.swapChainTarget)
        {
            framebuffer =
              framebuffers
                [VulkanContext::get()->getSwapChain()->getCurrentImageIndex()]
                  .get();
        }
    ...
    auto VulkanPipeline::bind(const CommandBuffer* cmdBuffer,
                              uint32_t layer,
                              int32_t cubeFace,
                              int32_t mipMapLevel) -> FrameBuffer*
    {
        PROFILE_FUNCTION();
        FrameBuffer* framebuffer = nullptr;
        transitionAttachments();
        ...
        if (depthBiasEnabled)
            vkCmdSetDepthBias(static_cast<const VulkanCommandBuffer*>(cmdBuffer)
                                ->getCommandBuffer(),
                              depthBiasConstant,
                              0.0f,
                              depthBiasSlope);
        ...
        {
            cmdBuffer->updateViewport(getWidth() * mipScale,
                                      getHeight() * mipScale);
        }

        vkCmdBindPipeline(static_cast<const VulkanCommandBuffer*>(cmdBuffer)
                            ->getCommandBuffer(),
    ...
    auto VulkanPipeline::bind(const CommandBuffer* cmdBuffer,
                              const ivec4& viewport) -> FrameBuffer*
    {
        PROFILE_FUNCTION();
        FrameBuffer* framebuffer = nullptr;
        transitionAttachments();
    ...
    auto VulkanPipeline::clearRenderTargets(const CommandBuffer* commandBuffer)
      -> void
    {
        PROFILE_FUNCTION();
        if (description.swapChainTarget)
        {
            for (uint32_t i = 0; i < GraphicsContext::get()
                                       ->getSwapChain()
                                       ->getSwapChainBufferCount();
                 i++)
            {
                RenderDevice::get()->clearRenderTarget(
                  GraphicsContext::get()->getSwapChain()->getImage(i),
                  commandBuffer);
            }
        }
        ...
        if (description.depthArrayTarget)
        {
            RenderDevice::get()->clearRenderTarget(description.depthArrayTarget,
                                                   commandBuffer);
        }

        if (description.depthTarget)
        {
            RenderDevice::get()->clearRenderTarget(description.depthTarget,
                                                   commandBuffer);
        }
    ...
    auto VulkanPipeline::dispatchIndirect(const CommandBuffer* cmdBuffer,
                                          ...
                                          uint64_t offset) -> void
    {
        ...
        vkCmdDispatchIndirect(
          vkCmd->getCommandBuffer(), vkBuffer->getHandle(), offset);
    }

    auto VulkanPipeline::drawIndexedIndirect(const CommandBuffer* cmdBuffer,
                                             const StorageBuffer* ssbo,
                                             uint32_t drawCount,
                                             uint32_t stride,
                                             uint64_t offset /*= 0*/) -> void
    {
        ...
        auto vkCmd = static_cast<const VulkanCommandBuffer*>(cmdBuffer);
        auto vkBuffer = static_cast<const VulkanStorageBuffer*>(ssbo);
        vkCmdDrawIndexedIndirect(vkCmd->getCommandBuffer(),
                                 vkBuffer->getHandle(),
                                 offset,
                                 drawCount,
                                 stride);
    ...
    auto VulkanPipeline::drawIndexed(const CommandBuffer* cmdBuffer,
                                     uint32_t indexCount,
                                     ...
                                     uint32_t firstInstance) -> void
    {
        PROFILE_FUNCTION();
        auto vkCmd = static_cast<const VulkanCommandBuffer*>(cmdBuffer);
        vkCmdDrawIndexed(vkCmd->getCommandBuffer(),
                         indexCount,
                         instanceCount,
                         firstIndex,
                         vertexOffset,
    ...
    auto VulkanPipeline::bufferBarrier(
    ...
    auto VulkanPipeline::transitionAttachments() -> void
    {
        ...
        auto commandBuffer = static_cast<VulkanCommandBuffer*>(
          GraphicsContext::get()->getSwapChain()->getCurrentCommandBuffer());
        ...
        if (description.swapChainTarget)
        {
            for (uint32_t i = 0; i < GraphicsContext::get()
                 ...
                 i++)
            {
                ((VulkanTexture2D*)GraphicsContext::get()
                   ->getSwapChain()
                   ->getImage(i)
                   .get())
        ...
        if (description.depthTarget)
        {
            ((VulkanTextureDepth*)description.depthTarget.get())
              ->transitionImage(
                VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL,
                commandBuffer);
        }
        ...
        for (auto texture : description.colorTargets)
        {
            if (texture != nullptr)
            {
                ...
                {
                    ((VulkanTextureDepth*)texture.get())
                      ->transitionImage(
                        VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL,
                        commandBuffer);
                }
    ...
    auto VulkanPipeline::createFrameBuffers() -> void
    {
        ...
        if (description.swapChainTarget)
        {
            textures.emplace_back(
              GraphicsContext::get()->getSwapChain()->getCurrentImage());
        }
        else
        {
            for (auto texture : description.colorTargets)
        ...
        {
            auto bufferCount =
              GraphicsContext::get()->getSwapChain()->getSwapChainBufferCount();
