...
namespace maple
{
    namespace
    {
        inline auto getStrideFromVulkanFormat(VkFormat format) -> uint32_t
        {
            switch (format)
            {
                ...
                case VK_FORMAT_R16G16B16A16_SFLOAT:
                    return sizeof(float) * 2;
                default:
                    LOGE("Unsupported Vulkan Format {0} , {1} : {2}",
                         format,
                         __FUNCTION__,
                         __LINE__);
                    return 0;
        ...
        {
            VkFormat uintTypes[] = { VK_FORMAT_R32_UINT,
                                     VK_FORMAT_R32G32_UINT,
                                     VK_FORMAT_R32G32B32_UINT,
                                     VK_FORMAT_R32G32B32A32_UINT };

            VkFormat intTypes[] = { VK_FORMAT_R32_SINT,
                                    VK_FORMAT_R32G32_SINT,
                                    VK_FORMAT_R32G32B32_SINT,
                                    VK_FORMAT_R32G32B32A32_SINT };

            VkFormat floatTypes[] = { VK_FORMAT_R32_SFLOAT,
        ...
        inline auto read(const std::string& name)
          -> std::unique_ptr<std::vector<uint8_t>>
        {
            if (fileExists(name))
            {
                ...
                fileReader.seekg(0);
                fileReader.read(reinterpret_cast<char*>(buffer->data()),
                                fileSize);
                return buffer;
            }
            return nullptr;
    ...
    VulkanShader::VulkanShader(
      const std::vector<uint32_t>& vertData,
      const std::vector<uint32_t>& fragData,
      const std::unordered_set<std::string>& dynamicUniforms)
    ...
    VulkanShader::VulkanShader(const std::vector<uint32_t>& compData,
                               const VariableArraySize& size)
      : arraySize(size)
    {
    ...
    auto VulkanShader::bindPushConstants(const CommandBuffer* cmdBuffer,
    ...
    auto VulkanShader::init(const ShaderTypes& path) -> void
    {
        ...
        for (auto& source : sources)
        {
            if (source.second == "null")
            {
                ...
                shaderStages[currentShaderStage].sType =
                  VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
                shaderStages[currentShaderStage].stage =
                  VkConverter::shaderTypeToVK(source.first);
    ...
    auto VulkanShader::createPipelineLayout() -> void
    {
        ...
        auto it = std::max_element(std::begin(descriptorLayoutInfo),
                                   std::end(descriptorLayoutInfo),
        ...
        for (auto& descriptorLayout : descriptorLayoutInfo)
        {
            ...
            if (raytracingShader) // raytracing shader usually shares same
                                  // pipeline layout
            {
                auto iter = std::find_if(
                  layouts[descriptorLayout.setID].begin(),
                  layouts[descriptorLayout.setID].end(),
                  [&](const DescriptorLayoutInfo& info)
                  {
                ...
                {
                    layouts[descriptorLayout.setID].emplace_back(
                      descriptorLayout);
                }
            }
            else
            {
                ...
                {
                    iter->stage = static_cast<ShaderType>(
                      static_cast<uint32_t>(iter->stage) |
        ...
        for (size_t i = 0; i < layouts.size(); i++)
        {
            ...
            for (uint32_t i = 0; i < l.size(); i++)
            {
                ...
                setLayoutBinding.descriptorType =
                  VkConverter::descriptorTypeToVK(info.type);
                setLayoutBinding.stageFlags =
                  VkConverter::shaderTypeToVK(info.stage);
                ...
                if (isArray)
                {
                    layoutBindingFlags.emplace_back(
                      info.variableSized
            ...
            flagsInfo.bindingCount =
              static_cast<uint32_t>(layoutBindingFlags.size());
            ...
            vkCreateDescriptorSetLayout(*VulkanDevice::get(),
                                        &setLayoutCreateInfo,
                                        VK_NULL_HANDLE,
                                        &layout);
        ...
        for (auto& pushConst : pushConstants)
        {
            ...
            pushConstantRanges.push_back(VulkanHelper::pushConstantRange(
              bits, pushConst.size, pushConst.offset));
        }
        ...
        VK_CHECK_RESULT(vkCreatePipelineLayout(*VulkanDevice::get(),
        ...
        for (auto& stage : shaderStages)
        {
            if (stage.module != nullptr)
                vkDestroyShaderModule(
                  *VulkanDevice::get(), stage.module, nullptr);
        }

        for (auto& descriptorLayout : descriptorSetLayouts)
            vkDestroyDescriptorSetLayout(
              *VulkanDevice::get(), descriptorLayout, VK_NULL_HANDLE);

        vkDestroyPipelineLayout(
          *VulkanDevice::get(), pipelineLayout, VK_NULL_HANDLE);
    }

    auto VulkanShader::loadShader(const std::vector<uint32_t>& spvCode,
                                  ShaderType shaderType,
                                  int32_t currentShaderStage) -> void
    {
        VkShaderModuleCreateInfo shaderCreateInfo{};
        shaderCreateInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
        shaderCreateInfo.codeSize =
          spvCode.size() * 4; // len is same as uint8_t
        ...
        if (shaderType == ShaderType::Vertex)
        {
            ...
            std::sort(copyInput.begin(),
                      copyInput.end(),
                      [&](const auto& left, const auto& right)
                      {
                          auto location = comp.get_decoration(
                            left.id, spv::DecorationLocation);
                          auto location1 = comp.get_decoration(
                            right.id, spv::DecorationLocation);
                          return location < location1;
                      });

            for (const spirv_cross::Resource& resource : copyInput)
            {
                const spirv_cross::SPIRType& InputType =
                  comp.get_type(resource.type_id);
                VkVertexInputAttributeDescription description = {};
                description.binding =
                  comp.get_decoration(resource.id, spv::DecorationBinding);
                description.location =
                  comp.get_decoration(resource.id, spv::DecorationLocation);
                LOGV("Load Vertex , binding : {} location : {}, name : {}",
                     description.binding,
                     description.location,
                     name);
                ...
                vertexInputStride +=
                  getStrideFromVulkanFormat(description.format);
            }
        }
        ...
        {
            computeShader = true;
            localSizeX = comp.get_execution_mode_argument(
              spv::ExecutionMode::ExecutionModeLocalSize, 0);
            localSizeY = comp.get_execution_mode_argument(
              spv::ExecutionMode::ExecutionModeLocalSize, 1);
            localSizeZ = comp.get_execution_mode_argument(
        ...
        for (auto& resource : resources.storage_images)
        {
            ...
            if (glslType.basetype == spirv_cross::SPIRType::Image)
            {
                ...
                uint32_t set = comp.get_decoration(
                  resource.id, spv::DecorationDescriptorSet);
                uint32_t binding =
                  comp.get_decoration(resource.id, spv::DecorationBinding);

                auto iter =
                  std::find_if(descriptorInfos[set].begin(),
                               descriptorInfos[set].end(),
                               [&](const Descriptor& info)
                               {
                                   return info.type == DescriptorType::Image &&
                ...
                {
                    LOGI("Load Image, type {0}, set {1}, binding {2}, name {3}",
                         fmt,
                         set,
                         binding,
                         resource.name);
                    ...
                    descriptor.format =
                      spirvTypeToTextureType(glslType.image.format);
                }
            }
        }
        // Descriptor Layout
        ...
        {
            uint32_t set =
              comp.get_decoration(u.id, spv::DecorationDescriptorSet);
            uint32_t binding =
              comp.get_decoration(u.id, spv::DecorationBinding);
            auto& type = comp.get_type(u.type_id);

            LOGI(
              "Uniform {0} at set = {1}, binding = {2}", u.name, set, binding);

            descriptorLayoutInfo.push_back(
              { dynamicUniforms.count(u.name) > 0
            ...
            descriptor.offset = 0;
            descriptor.shaderType = shaderType;
            descriptor.type = dynamicUniforms.count(u.name) > 0
                                ? DescriptorType::UniformBufferDynamic
                                : DescriptorType::UniformBuffer;
            ...
            for (int32_t i = 0; i < memberCount; i++)
            {
                ...
                const auto& memberName =
                  comp.get_member_name(bufferType.self, i);
        ...
        for (auto& u : resources.push_constant_buffers)
        {
            ...
            uint32_t binding =
              comp.get_decoration(u.id, spv::DecorationBinding);
            uint32_t binding3 =
              comp.get_decoration(u.id, spv::DecorationOffset);
            ...
            {
                    LOGI("Accessing Member {0} offset {1}, size {2}",
            range.index, range.offset, range.range); size +=
...
*/
            ...
            auto iter = std::find_if(pushConstants.begin(),
                                     pushConstants.end(),
                                     [&](const PushConstant& consts) {
                                         return consts.name == u.name &&
                                                bufferSize == consts.size;
                                     });
            ...
            if (iter == pushConstants.end())
            {
                ...
                for (int32_t i = 0; i < memberCount; i++)
                {
                    ...
                    const auto& memberName =
                      comp.get_member_name(bufferType.self, i);
                    auto size =
                      comp.get_declared_struct_member_size(bufferType, i);
                    auto offset = comp.type_struct_member_offset(bufferType, i);
                ...
                LOGI(
                  "Push Constant {0} at set = {1}, binding = {2}, size = {3}",
                  u.name.c_str(),
                  set,
                  binding,
                  totalSize);
        ...
        for (auto& u : resources.sampled_images)
        {
            ...
            auto iter = std::find_if(
              descriptorInfos[set].begin(),
              descriptorInfos[set].end(),
              [&](const Descriptor& info)
              {
                  return info.type == DescriptorType::ImageSampler &&
                         info.binding == binding && info.name == u.name;
              });
            ...
            if (iter == descriptorInfos[set].end())
            {
                ...
                auto& type = comp.get_type(u.type_id);
                LOGI("Found Sampled Texture {0} at set = {1}, binding = {2}",
                     u.name.c_str(),
                     set,
                     binding);
                ...
                descriptorLayoutInfo.push_back({ DescriptorType::ImageSampler,
                                                 shaderType,
                                                 binding,
                                                 set,
                                                 size,
        ...
        for (auto& u : resources.storage_buffers)
        {
            ...
            LOGI(
              "Buffer {0} at set = {1}, binding = {2}", u.name, set, binding);
            auto& type = comp.get_type(u.type_id);
            ...
            auto iter2 =
              std::find_if(descriptorInfos[set].begin(),
                           ...
                           [&](const Descriptor& info)
                           {
                               return info.type == DescriptorType::Buffer &&
                                      info.binding == binding &&
                                      info.name == u.name;
                           });

            auto iter =
              std::find_if(descriptorLayoutInfo.begin(),
                           descriptorLayoutInfo.end(),
                           [&](const DescriptorLayoutInfo& info)
                           {
                               return info.type == DescriptorType::Buffer &&
                                      info.binding == binding &&
                                      info.setID == set && info.name == u.name;
                           });

            if (iter == descriptorLayoutInfo.end() &&
                iter2 == descriptorInfos[set].end())
            {
                descriptorLayoutInfo.push_back({ DescriptorType::Buffer,
                                                 shaderType,
                                                 binding,
                                                 set,
                                                 size,
            ...
            { // todo raytracing shader check..
                auto mask = static_cast<int32_t>(iter->stage) |
        ...
        for (auto& u : resources.acceleration_structures)
        {
            ...
            LOGI("Acceleration Structures {0} at set = {1}, binding = {2}",
                 u.name,
                 set,
                 binding);
            ...
            auto iter = std::find_if(
              ...
              [&](const Descriptor& info)
              {
                  return info.type == DescriptorType::AccelerationStructure &&
                         info.binding == binding && info.name == u.name;
              });
        ...
        shaderStages[currentShaderStage].sType =
          VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
        shaderStages[currentShaderStage].stage =
          VkConverter::shaderTypeToVK(shaderType);
        shaderStages[currentShaderStage].pName = "main";
        shaderStages[currentShaderStage].pNext = VK_NULL_HANDLE;

        VK_CHECK_RESULT(
          vkCreateShaderModule(*VulkanDevice::get(),
                               &shaderCreateInfo,
                               nullptr,
                               &shaderStages[currentShaderStage].module));
    }
