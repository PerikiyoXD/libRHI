...
namespace maple
{
    ...
    class NullAccelerationStructure : public AccelerationStructure
    {
      public:
        ...
        virtual auto updateTLAS(const mat4 &transform, uint32_t instanceId, uint32_t customInstanceId, uint64_t instanceAddress) -> uint64_t override { return 0; }

        virtual auto resetTLAS(uint32_t instanceId) -> void override {}

        virtual auto getDeviceAddress() const -> uint64_t override { return 0; }

        virtual auto mapHost() -> void * override { return nullptr; }

        virtual auto unmap() -> void override {}

        virtual auto copyToGPU(const CommandBuffer *cmd, uint32_t instanceSize, uint64_t offset) -> void override {}

        virtual auto copyToGPU(const CommandBuffer* cmd, const std::vector<BuildRange>& ranges) -> void override {};

        virtual auto build(const CommandBuffer *cmd, uint32_t instanceSize, uint32_t instanceOffset = 0) -> void override {};

        virtual auto isBuilt() const -> bool override { return true; };
    };        // namespace maple
